...
export const ChatHistorySidebar = ({
  ...
  const [creatingSession, setCreatingSession] = React.useState<boolean>(false);
  ...
  const [localTemp, setLocalTemp] = React.useState<number>(
    settings.temperature
  );
  const [localTopP, setLocalTopP] = React.useState<number>(settings.top_p);
  const [localMaxTokens, setLocalMaxTokens] = React.useState<number>(
    settings.max_tokens
  );
    ...
    <Sidebar variant="floating" className="pr-0">
      <SidebarHeader className=" rounded-t-lg dark:bg-slate-900">
        ...
        <Button
          ...
          className="w-full justify-center gap-2"
          variant="outline"
          disabled={creatingSession || deletingId !== null}
        >
          {creatingSession || deletingId ? (
                    ...
                    return (
                      <SidebarMenuItem key={chat.id}>
                        <SidebarMenuButton
                          ...
                          className={`group relative flex items-center p-1 hover:bg-slate/90 h-auto dark:bg-slate-800 active:scale-95 active:border active:border-green-400/90 transition-all ${
                            currentChatId === chat.id
                              ? "border border-green-400/90 dark:border-green-700/90"
                              : "border border-grey-400/90 dark:border-grey-700/90"
                          }`}
                        >
                          <div
                            className={`p-2 rounded ${categoryInfo.color} text-white`}
                          >
                    ...
                    return (
                      <SidebarMenuItem key={chat.id}>
                        <SidebarMenuButton
                          isActive={currentChatId === chat.id}
                          onClick={() => onSelectChat(chat.id)}
                          className={`group relative flex flex-col items-start gap-2 p-1.5 hover:bg-slate/90 h-auto dark:bg-slate-800 active:scale-95 active:border active:border-green-400/90 transition-all ${
                            currentChatId === chat.id
                              ? "border border-green-400/90 dark:border-green-700/90"
                              : "border border-grey-400/90 dark:border-grey-700/90"
                          }`}
                            ...
                            <div className="w-full space-y-2">
                              ...
                              <div className="flex items-center justify-between w-full">
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`p-1 rounded ${categoryInfo.color} text-white`}
                                  >
                                    <CategoryIcon className="h-3 w-3" />
                                  </div>
                              ...
                              <div className="flex items-center gap-2 w-full">
                                {!isEditingThis ? (
                                  ...
                                  <div className="flex items-center w-full gap-2">
                                    ...
                                    />
                                    <button
                                      className="h-7 w-7 flex items-center justify-center rounded bg-primary/10 text-primary"
                                    ...
                                    <button
                                      className="h-7 w-7 flex items-center justify-center rounded bg-muted/10 text-muted-foreground"
            ...
            >
              <Avatar className="h-8 w-8">
                ...
                <AvatarFallback className="bg-primary/10 text-primary">
                  {typeof ("" + (patient?.name || "")).charAt === "function"
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setLocalMaxTokens(parseInt(e.target.value || "0", 10))
                      }
                    />
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="h-4 w-4"
                    checked={localDebug}
                    onChange={(e) => setLocalDebug(e.target.checked)}
                  />
        ...
        <Dialog open={profileOpen} onOpenChange={setProfileOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Overview of the current patient and case
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className={getPriorityClasses(caseRecord?.priority)}>
                      Priority:{" "}
                      {caseRecord?.priority
                        ? caseRecord.priority.charAt(0).toUpperCase() +
                          caseRecord.priority.slice(1)
                        : "-"}
                    </span>
                  </div>
                  <div className="col-span-2">
              ...
              <Button
                onClick={async () => {
                  if (pendingDeleteId) {
                    ...
                    try {
                      await onDeleteChat(pendingDeleteId);
                    } finally {
                      setDeletingId(null);
                    }
                  }
